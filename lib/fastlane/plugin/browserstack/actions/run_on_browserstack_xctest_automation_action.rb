...
module Fastlane
  module Actions
    module SharedValues
      BROWSERSTACK_XCTEST_BUILD_ID ||= :BROWSERSTACK_XCTEST_BUILD_ID
    end
    class RunOnBrowserstackXctestAutomationAction < Action
      UPLOAD_API_ENDPOINT = "https://api-cloud.browserstack.com/app-automate/xcuitest/v2/build"
      SHARED_VALUE_NAME = "BROWSERSTACK_XCTEST_BUILD_ID"

      def self.run(params)
        args = params.values
        args[:shared_value_name] = SHARED_VALUE_NAME
        args[:run_xctest_api_endpoint] = UPLOAD_API_ENDPOINT

        browserstack_xctest_build_id = Helper::BrowserstackHelper.run_xctest_on_browserstack(args)

        # Setting app id in SharedValues, which can be used by other fastlane actions.
        Actions.lane_context[SharedValues::BROWSERSTACK_XCTEST_BUILD_ID] = browserstack_xctest_build_id.to_s
      end

      def self.description
        "Launches XCTest automation run on BrowserStack."
      end
      ...
      def self.output
        [
          ['BROWSERSTACK_XCTEST_BUILD_ID', 'ID of launched automation run.']
        ]
      end
      ...
      def self.available_options
        [
          ...
          FastlaneCore::ConfigItem.new(key: :devices,
                                       description: "Array of the devices to launch XCTest automation run on",
                                       optional: false,
                                       is_string: false,
                                       verify_block: proc do |value|
          ...
          FastlaneCore::ConfigItem.new(key: :skip_testing,
                                       ...
                                       optional: true,
                                       is_string: false),
          FastlaneCore::ConfigItem.new(key: :enable_result_bundle,
                                       description: "Enable generating result bundles for XCTest build execution",
                                       optional: true,
                                       is_string: false),
        ]
      end
      ...
      def self.example_code
        [
          'run_on_browserstack_xctest_automation(
